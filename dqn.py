...
# class Memory:
#     def __init__(self, max_size: int, state_size: int, multi_step: int, history_len: int, batch_size: int, epsilon: float, prioritize: bool):
...
#         self.cursor = 0
#
#         self.states     = np.zeros((self.max_size, state_size), dtype=float)
#         self.actions    = np.zeros(self.max_size, dtype=int)
#         self.rewards    = np.zeros(self.max_size, dtype=float)
...
#             if self.size < self.max_size:  # sample just from filled so far
#                 p = p[:self.size]
#         else:
#             p = np.ones(self.size)
...
#
#     # TODO try logcosh loss, n_steps, idealization, per, selu, alpha dropout, he_uniform, regularization
#     def __init__(
#         self, env, seed=24,
